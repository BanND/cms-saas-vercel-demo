...
export const getInstance = cache(async () => {
  ...
  if (!fx) throw new Error("Optimizely Feature Experimentation not created");
  ...
  try {
    ...
    if (!success)
      throw new Error(
        "Optimizely Feature Experimentation initialization failed: " +
          (reason ?? ""),
      );
  } catch (error) {
    throw new Error(
      "Optimizely Feature Experimentation initialization failed: " + error,
    );
  }
  return fx;
});
